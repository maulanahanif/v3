...
print_install "Membuat direktori xray"
    ...
    export ISP=$(cat /etc/xray/isp)
    export CITY=$(cat /etc/xray/city)
...
rm -rf /etc/vmess/.vmess.db
    ...
    mkdir -p /etc/kyt/log/ssh/ssh.log
    mkdir -p /etc/kyt/log/vmess/vmess.log
    mkdir -p /etc/kyt/log/vless/vless log
    mkdir -p /etc/kyt/log/trojan/trojan.log
...
cat >/etc/cron.d/limit_ssh <<-END
		SHELL=/bin/sh
		PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
		*/10 * * * * root /usr/local/sbin/limit-ip-ssh
    END
...
clear
    ...
    echo "    │   - XRAY (DNSTT/SLOWDNS)    : 53, 5300              │"
